...
**Berdasarkan hasil seleksi berkas dan tes akademik:** melalui surat resmi ini, panitia PPDB {{ $sekolah?->nama ?? 'sekolah' }} menyatakan bahwa Anda telah memenuhi kriteria dan **DITERIMA** sebagai siswa baru untuk tahun ajaran {{ now()->format('Y') }}/{{ now()->addYear()->format('Y') }}.
...
<div style="border: 1px solid #111; padding: 10px; margin: 25px 0; text-align: center; font-family: 'Courier New', Courier, monospace; font-weight: bold;">
    NOMOR REGISTRASI: PPDB-{{ $record->id }}-{{ now()->format('Y') }}
</div>

**Integritas Akademik:** Seluruh data yang dikirimkan harus merupakan data asli. Jika ditemukan ketidaksesuaian data di kemudian hari, maka status penerimaan ini dapat dibatalkan secara sepihak oleh sekolah.
