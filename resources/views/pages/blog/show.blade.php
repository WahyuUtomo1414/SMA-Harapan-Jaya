...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-8 -mt-10 mb-20">
        <div class="aspect-[21/9] overflow-hidden shadow-2xl">
            <img src="{{ $blog->foto }}" alt="{{ $blog->title }}" class="w-full h-full object-cover" />
        </div>
    </div>
        ...
        <div class="max-w-7xl mx-auto px-8">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-20">
                ...
                <div class="lg:col-span-4 space-y-16">
                    ...
                    <div>
                        <span class="block text-on-surface font-headline font-bold text-xl mb-10 italic">Artikel Terkait</span>
                            ...
                            @foreach($relatedBlogs as $related)
                                <a href="{{ route('blog.show', $related->slug) }}" class="group flex gap-6 items-start">
                                    <div class="w-20 h-20 flex-shrink-0 overflow-hidden">
                                        <img src="{{ $related->thumbnail }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                    </div>
                                    <div>
                                        <span class="block text-primary font-subhead font-bold text-[9px] tracking-widest uppercase mb-1">{{ $related->kategori->nama }}</span>
                                        <h4 class="font-headline font-bold text-sm text-on-surface group-hover:text-primary transition-colors leading-tight line-clamp-2">{{ $related->title }}</h4>
                                    </div>
                                </a>
                            @endforeach
                        </div>
